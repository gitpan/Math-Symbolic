...
$str =~ s/\s+//g;
ok(
  $str eq '(x*2)+1' || $str eq '1+(2*x)', 'Correct result of overloaded *,+',
);
...
$str =~ s/\s+//g;
ok(
  $str eq '(x/2)-1', 'Correct result of overloaded /,-',
);
...
$str =~ s/\s+//g;
ok(
  $str eq 'x*(1+2)' || $str eq 'x*(2+1)' ||
  $str eq '(2+1)*x' || $str eq '(1+2)*x',
...
ok(
  ($seventh?0:1), 'automatic boolean conversion (Test1)'
);

ok(
  ($second?1:0), 'automatic boolean conversion (Test2)'
);
